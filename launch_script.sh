...
if [ "$PUB_ALL" == "false" ]; then
    tmux send-keys "ros2 launch launch/trafficlightbridge.launch.xml carla_host:=172.17.01 pub_all:=false" C-m
else
    tmux send-keys "ros2 launch launch/trafficlightbridge.launch.xml carla_host:=172.17.01" C-m
fi